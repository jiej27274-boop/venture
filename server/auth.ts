...
export async function createSession(input: { userId: string; organizationId: string }, database: VentureDatabase) {
...
export async function readSession(token: string | undefined, database: VentureDatabase) {
  ...
  try {
    ...
    const stored = await readAuthSessionRecord(database, hashOpaqueValue(token), parsed.sub, parsed.org);
