...
export default function App({ projectId }: { projectId?: string }) {
  useEffect(() => { if (!projectId && window.location.pathname !== "/") window.history.replaceState({}, "", `/${window.location.hash}`); }, [projectId]);
  ...
  const go = (next: View, query = "") => { setSelectedTelegraph(undefined); setView(next); setSearchQuery(query); window.location.hash = next; if (projectId) window.history.replaceState({}, "", viewToPath[next]); window.scrollTo({ top: 0, behavior: "smooth" }); setMenuOpen(false); };
