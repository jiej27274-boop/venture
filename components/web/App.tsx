...
function ProjectDetailPage({ projectId, go }: { projectId: string; go: (view: View, query?: string) => void }) {
  ...
  return <main className="page project-detail-page">{loadError ? <div className="section-wrap project-detail-state"><div className="loading error">加载失败：{loadError}</div><button className="outline" onClick={() => go("projects")}>返回项目库</button></div> : !project ? <div className="section-wrap project-detail-state"><div className="loading">正在加载项目…</div></div> : <><section className="project-detail-hero"><div className="section-wrap project-detail-hero-inner"><button className="project-detail-back" onClick={() => go("projects")}>← 返回项目库</button><div className="project-detail-hero-top"><div><span className="eyebrow">PROJECT PROFILE</span><h1>{project.name}</h1><p>{project.summary}</p><div className="project-detail-tags"><span>{project.industry}</span><span>{project.region}</span><span>{project.stage}</span><em>{project.identityMode === "anonymous" ? "匿名公开" : "实名公开"}</em></div></div><div className="project-detail-mark" aria-hidden="true">{project.name.trim().slice(0, 1) || "V"}</div></div><div className="project-detail-stats"><div><small>融资阶段</small><b>{project.stage}</b></div><div><small>所在地区</small><b>{project.region}</b></div><div><small>融资需求</small><b>{project.financingRange}</b></div></div></div></section><section className="section-wrap project-detail-body"><article className="project-detail-panel"><span className="eyebrow">OVERVIEW</span><h2>项目概览</h2><p className="project-summary">{project.summary}</p><div className="project-detail-grid"><div><small>所属行业</small><b>{project.industry}</b></div><div><small>所在地区</small><b>{project.region}</b></div><div><small>融资阶段</small><b>{project.stage}</b></div><div><small>融资需求</small><b>{project.financingRange}</b></div></div><div className="bp-notice"><span>BP</span><div><b>商业计划书受授权保护</b><p>请填写查看用途，项目方审核后开放材料访问。</p></div></div></article><aside className="project-detail-request"><span className="eyebrow">BP ACCESS</span><h2>申请查看 BP</h2><p className="request-note">填写查看用途，项目方审核通过后开放材料访问。</p>{state === "success" ? <div className="bp-request-success"><b>申请已提交</b><p>项目方审核后，你会获得授权通知。</p></div> : <form className="bp-request-form" onSubmit={submit}><textarea required minLength={10} value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="请说明查看 BP 的用途（至少 10 字）"/>{state === "error" && <p className="form-error">提交失败，请确认已完成主体认证。</p>}<button className="primary submit" disabled={state === "submitting"}>{state === "submitting" ? "提交中…" : "提交 BP 查看申请"}</button></form>}</aside></section></>}</main>;
}
