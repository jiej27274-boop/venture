...
const organizationType = { company: "公司", investor: "投资机构", fa: "FA 机构", government: "政府招商" } as const;
...
function PortalHero({ projects, organizations, contacts, go }: {
  ...
  const stats = [
    ...
    { value: organizations.filter((organization) => organization.type !== "company").length, label: "入驻机构", tone: "blue", icon: "institution" as const },
...
function PortalDataDashboard({ projects, organizations, go }: {
  ...
  const rankedOrganizations = organizations.filter((organization) => organization.type !== "company").slice(0, 5);
...
function OrganizationsView({ organizations, initialQuery = "", favoriteKeys, onToggleFavorite }: { organizations: Organization[]; initialQuery?: string; favoriteKeys: Set<string>; onToggleFavorite: (type: FavoriteResourceType, id: string) => void }) {
  ...
  const [companyStatus, setCompanyStatus] = useState<"" | "未上市" | "新股上会" | "IPO上市" | "新三板" | "估值">("");
  const [round, setRound] = useState("");
  const [timeFilter, setTimeFilter] = useState("");
  const [foundedQuery, setFoundedQuery] = useState("");
  useEffect(() => setQ(initialQuery), [initialQuery]);
  const normalized = q.trim().toLowerCase();
  const normalizedHeadquarters = headquartersQuery.trim().toLowerCase();
  const normalizedFounded = foundedQuery.trim().toLowerCase();
  const companyOrganizations = organizations.filter((organization) => organization.type === "company");
  ...
  const statusMatches = (organization: Organization) => !companyStatus ? true : companyStatus === "估值" ? Boolean(organization.valuation) : organization.companyStatus === companyStatus;
  const roundMatches = (organization: Organization) => !round || round === "全部" || organization.latestRound === round;
  const timeMatches = (organization: Organization) => {
    if (!timeFilter || timeFilter === "全部") return true;
    const at = organization.lastFundingAt ?? "";
    if (timeFilter === "2024" || timeFilter === "2025" || timeFilter === "2026") return at.startsWith(timeFilter);
    const months = timeFilter === "近三个月" ? 3 : timeFilter === "最近半年" ? 6 : 12;
    if (!/^\d{4}-\d{2}$/.test(at)) return false;
    const [year, month] = at.split("-").map(Number);
    const then = new Date(year, month - 1, 1);
    const now = new Date();
    const diff = (now.getFullYear() - then.getFullYear()) * 12 + (now.getMonth() - then.getMonth());
    return diff >= 0 && diff < months;
  };
  const foundedMatches = (organization: Organization) => !normalizedFounded || (organization.foundedYear ?? "").toLowerCase().includes(normalizedFounded);
  const filtered = companyOrganizations.filter((organization) =>
    statusMatches(organization) &&
    industryMatches(organization) &&
    regionMatches(organization) &&
    roundMatches(organization) &&
    timeMatches(organization) &&
    foundedMatches(organization) &&
  ...
  const clearFilters = () => { setIndustry(""); setRegion(""); setQ(""); setHeadquartersQuery(""); setCompanyStatus(""); setRound(""); setTimeFilter(""); setFoundedQuery(""); };
  const statusForSidebar: Record<string, "" | "未上市" | "新股上会" | "IPO上市" | "新三板" | "估值"> = { "全部公司": "", "未上市公司": "未上市", "新股上会公司": "新股上会", "IPO上市公司": "IPO上市", "新三板": "新三板", "公司估值": "估值" };
  const statusCounts: Record<string, number> = {
    "全部公司": companyOrganizations.length,
    "未上市": companyOrganizations.filter((organization) => organization.companyStatus === "未上市").length,
    "新股上会": companyOrganizations.filter((organization) => organization.companyStatus === "新股上会").length,
    "IPO上市": companyOrganizations.filter((organization) => organization.companyStatus === "IPO上市").length,
    "新三板": companyOrganizations.filter((organization) => organization.companyStatus === "新三板").length,
    "估值": companyOrganizations.filter((organization) => Boolean(organization.valuation)).length,
  };
  const companyTitle = companyStatus ? (companySidebarItems.find((item) => statusForSidebar[item] === companyStatus) ?? "全部公司") : "全部公司";
  return <main className="page company-directory-page"><section className="section-wrap company-directory-shell"><div className="company-directory-layout">
    <aside className="company-sidebar" aria-label="公司分类导航"><div className="company-sidebar-heading"><span>COMPANY INDEX</span><h1>公司</h1><p>公司主体、上市状态与估值信息。</p></div><nav className="company-sidebar-nav">{companySidebarItems.map((item) => <button type="button" key={item} className={companyStatus === statusForSidebar[item] || (!companyStatus && item === "全部公司") ? "active" : ""} onClick={() => setCompanyStatus(statusForSidebar[item])}><StatIcon name={item === "全部公司" ? "company" : item === "IPO上市公司" ? "rocket" : "industry"}/><span className="company-sidebar-item-label">{item}</span><small>{statusCounts[statusForSidebar[item] || "全部公司"]}</small></button>)}</nav><div className="company-sidebar-note">数据来自公开信息与平台核验，点击分类即可筛选。</div></aside>
    <div className="company-workspace"><div className="company-filter-panel"><div className="company-filter-row"><span className="company-filter-label">睿兽行业</span><div className="company-filter-options">{companyIndustryOptions.map((value, index) => <button type="button" key={value} className={(!industry && index === 0) || industry === value ? "active" : ""} onClick={() => setIndustry(index === 0 ? "" : value)}>{value}</button>)}</div></div><div className="company-filter-row"><span className="company-filter-label">最新轮次</span><div className="company-filter-options">{companyRoundOptions.map((value) => <button type="button" key={value} className={(!round && value === "全部") || round === value ? "active" : ""} onClick={() => setRound(value === "全部" ? "" : value)}>{value}</button>)}</div></div><div className="company-filter-row"><span className="company-filter-label">公司地区</span><div className="company-filter-options">{["全部", "中国", "海外"].map((value) => <button type="button" key={value} className={(!region && value === "全部") || region === value ? "active" : ""} onClick={() => setRegion(value === "全部" ? "" : value as "中国" | "海外")}>{value}</button>)}</div></div><div className="company-filter-row"><span className="company-filter-label">获投时间</span><div className="company-filter-options">{companyTimeOptions.map((value) => <button type="button" key={value} className={(!timeFilter && value === "全部") || timeFilter === value ? "active" : ""} onClick={() => setTimeFilter(value === "全部" ? "" : value)}>{value}</button>)}</div></div><div className="company-filter-row company-filter-row-last"><span className="company-filter-label">其他指标</span><div className="company-other-filters"><label><span>成立时间</span><input value={foundedQuery} onChange={(event) => setFoundedQuery(event.target.value)} placeholder="输入成立年份，如 2015" aria-label="成立时间"/></label><label><span>投资方总部</span><input value={headquartersQuery} onChange={(event) => setHeadquartersQuery(event.target.value)} placeholder="请输入地区名称" aria-label="投资方总部"/></label><label><span>标签搜索</span><input value={q} onChange={(event) => setQ(event.target.value)} placeholder="请搜索企业画像标签" aria-label="标签搜索"/></label></div></div></div>
      <section className="company-results-panel"><div className="company-results-heading"><div><div className="company-results-title"><h2>{companyTitle}</h2><span>{filtered.length} 家结果</span></div><p>{q ? <>搜索“{q}”的匹配结果</> : "公开展示已完成基础核验的公司信息"}</p></div><span className="company-results-status"><i/>已核验数据</span></div><div className="company-table-wrap"><table className="company-table"><thead><tr><th className="company-check-col">对比</th><th className="company-index-col">序号</th><th>公司</th><th>一句话简介</th><th>行业领域</th><th>地区</th><th>上市状态</th><th>成立时间</th><th className="company-action-col">收藏</th></tr></thead><tbody>{filtered.map((organization, index) => <tr key={organization.id}><td className="company-check-col"><input type="checkbox" aria-label={"选择" + organization.name}/></td><td className="company-index-col">{String(index + 1).padStart(2, "0")}</td><td><div className="company-name-cell"><div className={"company-table-mark " + organization.type}>{organization.name.slice(0, 1)}</div><div><strong>{organization.name}</strong><small>{organizationType[organization.type]}</small></div></div></td><td className="company-tagline-cell">{organization.tagline}</td><td><div className="company-focus-cell">{organization.focus.slice(0, 2).map((value) => <span key={value}>{value}</span>)}</div></td><td>{organization.region}</td><td><span className={"company-status-badge " + (organization.companyStatus || "未上市")}>{organization.companyStatus || "未上市"}</span></td><td>{organization.foundedYear || "—"}</td><td className="company-action-col"><FavoriteButton active={favoriteKeys.has("organization:" + organization.id)} onClick={() => onToggleFavorite("organization", organization.id)}/></td></tr>)}</tbody></table>{filtered.length === 0 && <div className="company-empty"><strong>没有匹配的公司</strong><p>调整行业、轮次、地区或关键词后再试。</p><button className="outline" onClick={clearFilters}>清除筛选</button></div>}</div></section></div>
  </div></section></main>;
}
...
function InstitutionsView({ organizations, favoriteKeys, onToggleFavorite }: { organizations: Organization[]; favoriteKeys: Set<string>; onToggleFavorite: (type: FavoriteResourceType, id: string) => void }) {
  ...
  const normalizedFounded = foundedQuery.trim().toLowerCase();
  const normalizedStage = stageQuery.trim().toLowerCase();
  const industryAliases: Record<string, string[]> = { "智能制造": ["智能制造", "先进制造", "机器人工"], "消费": ["消费", "消费科技"] };
  const industryMatches = (organization: Organization) => !industry || industry === "全部" || organization.focus.some((value) => (industryAliases[industry] ?? [industry]).some((alias) => value.includes(alias) || alias.includes(value)));
  const regionMatches = (organization: Organization) => !region || (region === "中国" ? !organization.region.includes("海外") : organization.region.includes("海外"));
  ...
  const filtered = institutionOrganizations.filter((organization) =>
    ...
    industryMatches(organization) &&
    regionMatches(organization) &&
    (!fundCurrency || (organization.fundCurrency ?? "").includes(fundCurrency)) &&
    (!investmentStage || (organization.investmentStage ?? "").includes(investmentStage)) &&
    (!normalizedFounded || (organization.foundedYear ?? "").includes(normalizedFounded)) &&
    (!normalizedStage || (organization.investmentStage ?? "").includes(normalizedStage)) &&
    (!esgOnly || organization.esg) &&
...
const authRoleLabels: Record<string, string> = { user: "普通用户", company: "公司", project: "项目方", investor: "投资机构", fa: "FA 机构", government: "政府招商", platform: "平台管理员" };
